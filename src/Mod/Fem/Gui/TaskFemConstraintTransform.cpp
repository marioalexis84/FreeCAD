    ...
    , ui(new Ui_TaskFemConstraintTransform)
{
    ...
    Base::Vector3d axis;
    double angle;
    pcConstraint->RectangularTransform.getValue().getValue(axis, angle);
    ...
    ui->spb_rot_axis_x->bind(
        App::ObjectIdentifier::parse(pcConstraint, std::string("RectangularTransform.Axis.x")));
    ui->spb_rot_axis_y->bind(
        App::ObjectIdentifier::parse(pcConstraint, std::string("RectangularTransform.Axis.y")));
    ui->spb_rot_axis_z->bind(
        App::ObjectIdentifier::parse(pcConstraint, std::string("RectangularTransform.Axis.z")));
    ui->qsb_rot_angle->bind(
        App::ObjectIdentifier::parse(pcConstraint, std::string("RectangularTransform.Angle")));
...
void TaskFemConstraintTransform::xAxisChanged(double x)
{
    ...
    Fem::ConstraintTransform* pcConstraint =
        static_cast<Fem::ConstraintTransform*>(ConstraintView->getObject());
    pcConstraint->RectangularTransform.setValue(rot);
...
bool TaskDlgFemConstraintTransform::accept()
{
    ...
    try {
        ...
        Gui::Command::doCommand(
            Gui::Command::Doc,
            "App.ActiveDocument.%s.RectangularTransform = App.Rotation(App.Vector(%f,% f, %f), Radian=%f)",
            name.c_str(),
            axis.x,
            axis.y,
            axis.z,
