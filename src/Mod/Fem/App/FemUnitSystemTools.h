...
const std::map<std::string, const BasicUnits> coherentSymbols = {
    ...
    {"MKS", {"m", "kg", "s", "A", "K", "mol", "cd", "rad", 0.001}},
};

inline double getCoherentLengthScale(const std::string& system)
{
    auto it = coherentSymbols.find(system);
    if (it == coherentSymbols.end()) {
        throw Base::ValueError(fmt::format("Invalid unit system: {}", system));
    }
    const BasicUnits& symbols = it->second;
    return symbols.lengthScale;
}

inline double getCoherentValue(const Base::Quantity& q, const std::string& system)
{
